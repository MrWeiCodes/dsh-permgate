...
> **⚠️ Note: renamed to `@mrweicodes/dsh-permgate` as of 1.3.9** (previously `dsh-permgate`).
> - **Installed via the plugin market**: click **Update** — the market performs the source migration automatically (removes the old name, installs the new one) and rolls back on failure.
> - **Manually installed (Option 3/4)**: remove and reinstall:
>   ```powershell
>   dsh plugin --profile web remove dsh-permgate
>   dsh plugin --profile web add -w github:MrWeiCodes/dsh-permgate
>   ```
> - Your configuration (`$DSH_HOME/dsh-permgate/config.json`) is unaffected.
...
2. Remove `"@mrweicodes/dsh-permgate": ...` from the `dependencies` of the profile's `package.json`.
...
- **Dependency & patch** (Option 4 installs): the `"@mrweicodes/dsh-permgate": ...` dependency in the profile's `package.json`, plus the `- insert: permgate` row and the `permission` preset override in `cordis.patch.yml`.
...
- **Compatible with [dsh-fs-encoding](https://github.com/MrWeiCodes/dsh-fs-encoding)**: DSH's filesystem contract is UTF-8-only, so an approval preview of a non-UTF-8 file (GBK / Big5 / Shift-JIS, …) cannot be read. When that plugin provides its `ctx.fsEncoding` decoding service, permgate's approval modal and comparison panel (including write-class diffs and undo previews) **reuse it** to decide the encoding and render the content, labelling the encoding it resolved; when the encoding was **guessed** rather than determined by a BOM, the preview marks it with a question mark so a guess is never presented as the file's real encoding. If the decoded text still contains NUL characters it is treated as binary and not previewed (the "cannot read" notice is kept instead of showing mojibake) — this test does not depend on how the encoding was decided: under guessing, binary content gets mapped by a single-byte encoding into text that merely looks decoded, and must be rejected too; legitimate UTF-16/32 (with BOM) text decodes without NULs and is unaffected. The dependency is **optional**: without the plugin the preview keeps its previous behaviour (reporting that a non-UTF-8 file cannot be read) and never guesses on its own, because two independent guesses over the same bytes would disagree about what a file says. The decision stays with that plugin. (Binary detection follows DSH's existing rule: only the first 8 KB are sampled, so a file whose NUL bytes appear after that window may still be shown as text — that is upstream `ctx.fs` behaviour.)
