...
- **设置 → 权限网关**：一站式管理所有权限——每个分类的默认行为、白/黑名单、快捷工具、自定义规则、底层沙箱，全局与项目分开配置，还能查看最近决策记录。
- **新会话默认权限设置**：在设置里把「自定义审查」设为新对话的默认权限，之后每次新建对话自动启用，不用手动重复选择。

  ![默认权限设置](assets/default-permission.jpg)
- **输入框下方快捷栏 / 会话权限选择器**：当前对话启用「自定义审查」时，输入框下方会显示六个分类的当前状态徽标，一眼看清 AI 现在能做什么。

  ![会话权限选择器](assets/permission-picker.jpg)
- **审批弹窗**：编辑/写入类审批默认展开 diff 详情、收起参数，快速判断改动是否合理；命令类审批直接给出可一键添加的规则候选（如 `git status *`），常放行的命令顺手就加进例外。

  ![审批弹窗](assets/approval-modal.jpg)
- **自定义拒绝意见**：拒绝时填写意见，AI 会收到「为什么不行、应该怎么做」的明确理由，立即调整方案。

  ![拒绝意见](assets/reject-reason.jpg)
