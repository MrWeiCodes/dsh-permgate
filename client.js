window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function EncBadge(props) {
			...
			// 浮层坐标（fixed 定位，相对视口）。null = 尚未测量，此时不渲染浮层。
			const [pos, setPos] = React.useState(null);
			useLocaleTick();
			const boxRef = React.useRef(null);
			const popRef = React.useRef(null);
			// 浮层必须是 fixed 且挂在原位置：它是 absolute 时会被 .pg2-block 的
			// overflow:hidden 裁剪——实测只看得见标题，候选与脚注全被切掉（diff 只有两行时
			// 块高约 64px，而浮层需要 120px+）。fixed 的包含块是视口，不受祖先 overflow 裁剪
			// （前提：祖先链上没有 transform/filter/will-change 之类会重建包含块的属性；
			// permgate 自己的样式里零命中，已实测确认）。
			// 代价是坐标要自己算，且滚动/resize 后需重算——见下面的 measure 与监听。
			const measure = () => {
				...
				// 横向：优先左对齐徽标；右侧放不下时贴右边（留 8px 余量），避免浮层出屏
				const width = popRef.current && popRef.current.offsetWidth ? popRef.current.offsetWidth : 240;
				let popLeft = r.left;
				if (vw && popLeft + width > vw - 8) popLeft = Math.max(8, vw - width - 8);
				// 纵向：默认在徽标下方；下方空间不足且上方更宽裕时翻到上方。
				// 变量名用 popTop/popLeft：这里同时存在 r.top/r.bottom（徽标矩形）与最终的
				// style.top，同名会让「哪一个是浮层坐标」难以分辨。
				const need = popRef.current && popRef.current.offsetHeight ? popRef.current.offsetHeight : 140;
				let popTop = r.bottom + 4;
				if (vh && popTop + need > vh - 8 && r.top - need - 4 > 8) popTop = r.top - need - 4;
				// 函数式更新 + 相等判断：滚动期 onMove 每帧都会调到这里，坐标没变时返回原引用
				// 让 React bail out，避免为「什么都没动」的帧重建一次元素树。
				setPos((p) => (p && p.left === popLeft && p.top === popTop) ? p : { left: popLeft, top: popTop });
			};
			React.useEffect(() => {
				if (!open) { setPos(null); return; }
				// 先渲染一次（pos 为 null 时用 visibility:hidden 占位以便量到尺寸），再测量定位
				measure();
				const onDoc = (e) => {
					// 徽标与浮层都在 boxRef 内判断：浮层是 fixed，但仍在 React 树内、DOM 上
					// 是 boxRef 的后代，故 contains 对它同样成立（无需额外排除浮层自身）
					if (boxRef.current && !boxRef.current.contains(e.target)) setOpen(false);
				};
				const onKey = (e) => { if (e.key === 'Escape') setOpen(false); };
				// 滚动/resize 后重算：fixed 坐标是视口坐标，页面滚动会让浮层与徽标脱位。
				// 用 rAF 合并：scroll 挂在捕获阶段，页面内任意滚动容器（含 .pg2-body）每滚动一次
				// 都会触发，而 measure 要同步读几何（强制布局）；不合并则开销按事件数而非帧数增长。
				// 写成 window.requestAnimationFrame(...) 的方法调用形式：与下面 cancelAnimationFrame
				// 同一写法，读起来也明确是宿主方法。注：rAF **不要求** this（实测取出后单独调用
				// 正常，不像 localStorage.getItem / history.pushState / document.createElement 那样
				// 抛 Illegal invocation），故这里只是风格选择，不是必需。
				let raf = 0;
				const onMove = () => {
					if (raf) return;
			...
			// 浮层与徽标是兄弟（不是父子）：浮层 fixed 定位后不再依赖父级位置，挂在 wrap 内
			// 会受 wrap 的尺寸影响；放同级更直观。boxRef 仍包住两者，供外部点击判定使用。
			return React.createElement('span', { className: 'pg2-enc-wrap', ref: boxRef },
				...
				open ? React.createElement('div', {
					className: 'pg2-enc-pop',
					ref: popRef,
					// 首帧还没量到坐标：先隐藏占位，量完再显示（否则会闪现在左上角）
